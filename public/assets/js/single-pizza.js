...
function getPizza() {
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    .then(response => {
      if (!response.ok) {
        throw new Error({ message: 'Something went very, very wrong.' });
      }
    ...
    .catch(err => {
      console.log(err);
      alert('Cannot find a pizza with this id. Taking you back to choose another');
      window.history.back();
    });
}
...
function handleNewCommentSubmit(event) {
  ...
  fetch(`/api/comments/${pizzaId}`, {
    ...
    .then(response => {
      if (!response.ok) {
        throw new Error('Something went wrong. Terribly wrong.');
      }
      response.json();
    })
